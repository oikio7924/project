...
CREATE TABLE IF NOT EXISTS bdm_plant_files (
  ...
  plant_id          INT UNSIGNED NOT NULL                       COMMENT 'bdm_plants.id',
  original_filename VARCHAR(500) NOT NULL                       COMMENT '업로드 당시 원본 파일명',
  file_size         INT UNSIGNED NOT NULL DEFAULT 0              COMMENT '파일 크기(byte)',
  uploaded_at       TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  INDEX idx_bdm_plant_files_plant (plant_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
  COMMENT='발전소별 첨부 서류. 실제 파일은 uploads/plant-docs/<plant_id>/<file_id>.<ext>에 저장';
