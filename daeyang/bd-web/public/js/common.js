...
/* 로그인 체크 — 보호된 페이지 맨 앞에서 호출. 미로그인이면 로그인 페이지로 이동 */
...
function bindLogout() {
  ...
  btn.addEventListener('click', async (e) => {
    e.preventDefault();
    try { await apiSend('api/logout', 'POST'); } catch (e) {}
    // 자동 로그인이 켜져 있으면 로그아웃 직후 login.html에서 바로 재로그인되어
    // 버려버리는 문제를 막기 위해, 로그아웃 시 자동 로그인 설정은 항상 끔
    try { localStorage.removeItem('bdweb_autoLogin'); } catch (e) {}
    window.location.href = 'login.html';
  });
}
...
const EVENT_COLORS = {
  'power-scheduled': '#b8dbfb',  /* 발전사업 허가예정 — 하늘파랑  */
  'power-permit':    '#8fbff5',  /* 발전사업 허가일   — 파랑      */
  'power-expire':    '#5c85c9',  /* 발전사업 만료일   — 진파랑    */
  'dev-permit':      '#ffc896',  /* 개발행위 허가일   — 주황      */
  'dev-expire':      '#e2818a',  /* 개발행위 만료일   — 빨강      */
  'ppa':             '#c4aef0',  /* PPA 접수일        — 보라      */
  'completion':      '#ffd59e',  /* 준공일            — 황토      */
  'operation':       '#a3e0c2',  /* 운전개시일        — 에메랄드  */
};
