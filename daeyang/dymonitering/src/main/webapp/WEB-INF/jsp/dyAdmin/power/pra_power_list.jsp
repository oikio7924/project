														...
														<div class="col-md-6">
															<select class="form-control DPP_USER" id="DPP_USER" name="DPP_USER">
																<c:forEach items="${member }" var="member">
																	<option value="${member.UI_KEYNO }" ${member.UI_ID eq 'daeyang0715' ? 'selected' : '' }>${member.UI_ID }</option>
																</c:forEach>
															</select>
														</div>
													</div>
...
function validation(){
	...
	var otherVolumEmpty = false;
	$(".other_volum").each(function(){
		if($.trim($(this).val()) == ''){ otherVolumEmpty = true; }
	});
	if(otherVolumEmpty){alert("인버터별 개별 용량을 모두 입력하세요!"); return false}
