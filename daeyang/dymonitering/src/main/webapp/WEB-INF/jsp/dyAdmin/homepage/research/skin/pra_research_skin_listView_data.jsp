...
<table id="dt_basic" class="pagingTable table table-striped table-bordered table-hover" width="100%">
	<thead>
		...
		<tr>
			...
			<th class="hasinput">
				<select class="form-control search-control" data-searchindex="7" onchange="pf_LinkPage()">
		...
		<tr>
			...
			<th class="arrow" data-index="6">수정일</th>
	...
	<tbody>
		...
		<fmt:parseNumber value="${fn:substring(model.PRS_KEYNO, 4, 20)}" var="KEY" />
		<tr>
			<td>${model.COUNT}</td>
			<td><a href="javascript:pf_researchSkinUpdate('${model.PRS_KEYNO}')"><c:out value="${model.PRS_SUBJECT} "/></a></td>
			<td><c:out value="${model.PRS_UI_NAME} "/></td>
			<td>${model.PRS_REGDT }</td>
			<c:choose>
				<c:when test="${not empty model.PRS_MODDT and not empty model.PRS_MODNM }">
				<td>${model.PRS_MODNM }</td>
				<td>${model.PRS_MODDT }</td>
				</c:when>
				<c:otherwise>
				<td colspan="2">수정기록 없음</td>			
				</c:otherwise>
			</c:choose>			
			<c:if test="${model.PRS_USE eq 'N'}">
			<td><font color="#FF0040"><b>X</b></font></td>
			</c:if>
			<c:if test="${model.PRS_USE eq 'Y'}">
			<td><font color="#29088A"><b>O</b></font></td>
			</c:if>			
			<td>
				<div class="btn btn-warning btn-xs" onclick="pf_PublishFile('${model.PRS_KEYNO}')">
