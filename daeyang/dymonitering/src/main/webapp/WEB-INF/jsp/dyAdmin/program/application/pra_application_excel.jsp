...
String fileName = "신청프로그램 리스트.xls";
...
<table>
	<thead>
		<tr></tr>
		<tr class="background">
			<th colspan="9" class="title">신청프로그램 리스트</th>
		</tr>
		<tr></tr>
		<tr>
			<th colspan="9" class="Subdate" style="text-align:left;">
...
<table id="dt_basic">
	<thead>
		<tr class="background2">
			...
			<th class="thsize">유형</th>
			<th class="thsize">요금</th>
			<th class="thsize">장소</th>
			<th class="thsize">등록자</th>
			<th class="thsize">등록일</th>
			<th class="thsize">사용여부</th>
			<th class="thsize">상태</th>
		</tr>
	</thead>
	<tbody>
		<c:forEach items="${resultList }" var="model" varStatus="status">
		<tr>
			<td class="tdName">${model.KEYNO }</td>
			<td class="tdName"><c:out value="${model.AP_NAME }"/></td>
			<td class="tdName">${model.TYPENAME }</td>
			<td class="tdName">${model.PRICE }</td>
			<td class="tdName"><c:out value="${model.PM_NAME }"/></td>
			<td class="tdName">${model.UI_NAME }</td>
			<td class="tdName">${model.AP_REGDT }</td>
			<td class="tdName">${model.AP_USE_NM }</td>
			<td class="tdName">
				<c:if test="${model.DEADLINE == '진행중'}">
