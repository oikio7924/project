...
String fileName = "메뉴별 통계 리스트.xls";
...
<table>
	<thead>
		<tr></tr>
		<tr class="background">
			<th colspan="${colspan}" class="title">메뉴별 통계 리스트</th>
		</tr>
		<tr></tr>
		<tr>
			...
			<th colspan="${colspan - 2}" class="Subdate">
				<c:if test="${not empty log.STDT || not empty log.ENDT  }">
				${log.STDT} ~ ${log.ENDT}
				</c:if>
			</th>
...
<table id="dt_basic">
	<thead>
		<tr class="background2">
			<th class="thsize">순위</th>
			<th class="thsize">홈페이지</th>
			<th class="thsize">메뉴</th>
			<th class="thsize">카운트</th>
			<th class="thsize">퍼센트</th>
		</tr>
	</thead>
	<tbody>
		<c:forEach items="${resultList }" var="model" varStatus="status">
		<tr>
			<td class="tdName">${model.no }</td>
			<td class="tdName">${model.MN_HOMEDIV_NAME }</td>
			<td class="tdName">${model.MN_NAME }</td>
			<td class="tdName">${model.selectCount }</td>
