...
function pf_formCheck(){
	...
	if($("select#BN_CATEGORY_NAME").val()=="전체"){
		cf_smallBox('Form', "카테고리를 선택해주세요.", 3000,'#d24158');
	...
	$('.BD_TYPE_VALIDATE').each(function(){
		...
		if(!state){
			if($(this).data('type') == 'check' || $(this).data('type') == 'radio'){
				...
				if(length < 1){
					cf_smallBox('Form', $(this).data('title')+"을 선택하여주세요.", 3000,'#d24158');
					$(window).scrollTop($(this).parent().find('input[name^=BD_DATA]').eq(0).next().offset().top - 100)
					state = true;
				}
			}else if(!$(this).val().trim()){
				cf_smallBox('Form', $(this).data('title')+"을 입력하여주세요.", 3000,'#d24158');
				$(this).focus();
				state = true;
			}
		}
	...
	$('.BD_DATA_EMAIL').each(function(){ //이메일 형식 체크
		...
		if(!state && value && !/^[0-9a-zA-Z]([-_\.]?[0-9a-zA-Z])*@[0-9a-zA-Z]([-_\.]?[0-9a-zA-Z])*\.[a-zA-Z]{2,3}$/i.test( value )){
			cf_smallBox('Form', $(this).data('title')+"의 이메일 형식이 올바르지 않습니다.", 3000,'#d24158');
			$(this).focus();
			state = true;
		}
	});
	...
	$('.BD_DATA_CALENDAR, .BD_DATA_CALENDAR_END, .BD_DATA_CALENDAR_START').each(function(){
		...
		if(!state && value && !/^(19|20)\d{2}-(0[1-9]|1[012])-(0[1-9]|[12][0-9]|3[0-1])$/.test( value )){
			cf_smallBox('Form', $(this).data('title')+'의 날짜 형식이 올바르지 않습니다.(YYYY-MM-DD)', 3000,'#d24158');
			$(this).focus();
			state = true;
		}
	})
	...
	if("${BoardType.BT_THUMBNAIL_YN}" == "Y"){
		if( action == 'insert' || ((action == 'update' || action == 'move' ) && !$('#BN_THUMBNAIL').val()) ){
			...
			if("${BoardType.BT_THUMBNAIL_INSERT}" == "N"){
				if(!$("#thumbnail").val()){
					cf_smallBox('Form', "썸네일 이미지를 선택해주세요.", 3000,'#d24158');
					return false;
				}	
			}
		}
	...
	if("${BoardType.BT_SECRET_YN }" == "Y"){
		if(!cf_radio_check_val("BN_SECRET_YN")){
			cf_smallBox('Form', "비밀글 사용여부를 선택해주세요.", 3000,'#d24158');
			return false;
		}		
	}
	...
	if(boardDetailCk){	//상세내용사용시
		...
		if( BN_CONTENTS == ""  || BN_CONTENTS == null || BN_CONTENTS == '&nbsp;' || BN_CONTENTS == '<p>&nbsp;</p>')  {
			 cf_smallBox('Form', "내용을 입력하세요.", 3000,'#d24158');
...
// 	if('${currentMenu.MN_GONGNULI_YN}' == 'Y'){ //공공누리
	...
	if(!$('#BN_KEYNO').val()){
		$.ajax({
			...
			error: function(jqXHR, exception) {
				cf_smallBox('error', "error.", 3000,'#d24158');
			}
		});
	}
}
...
function success(){
	cf_replaceTrim($("#Form"));
	if(action == 'insert'){
		$("#Form").attr("action", "/dyAdmin/homepage/board/data/action.do?${_csrf.parameterName}=${_csrf.token}");
	}else if(action == 'update'){
		$("#Form").attr("action", "/dyAdmin/homepage/board/data/action.do?${_csrf.parameterName}=${_csrf.token}");
	}else if(action == 'move'){
		$("#Form").attr("action", "/dyAdmin/homepage/board/data/move.do?${_csrf.parameterName}=${_csrf.token}");
	}else{
		cf_smallBox('error', "오류발생 : action값 없음.", 3000,'#d24158');
		return false;
	}
	$("#Form").submit();
}
function exit(){
	cf_smallBox('exit', "취소되었습니다.", 3000,'#d24158');
	return false;
}
...
function pf_initColumnData(BN_KEYNO){
	...
	$.ajax({
		type: "POST",
		url: "/dyAdmin/BoardData/updateView/listAjax.do",
...
function pf_adjust(id, bl_type, data){
	...
	if(bl_keyno){
		...
		cf_smallBox('empty', "선택된 데이터가 없습니다.", 3000,'#d24158');
	}
}
