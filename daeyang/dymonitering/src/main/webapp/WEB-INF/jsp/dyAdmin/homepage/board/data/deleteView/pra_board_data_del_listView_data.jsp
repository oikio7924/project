...
<c:set var="colspan" value="${fn:length(BoardColumnList)+8 }"/>
...
<table id="dt_basic" class="pagingTable table table-striped table-bordered table-hover" width="100%">
	<thead>
		...
		<tr>
			...
			<th class="hasinput">
			<select data-searchindex="2" class="form-control search-control" onchange="pf_LinkPage();">
		    	<option value="">전체 상위메뉴</option>
					<c:forEach items="${boardMain }" var="model">
						<option value="${model }">${model }</option>
					</c:forEach>
		    </select>
			</th>	
			<th class="hasinput">
			<select data-searchindex="3" class="form-control search-control" onchange="pf_LinkPage();">
		    	<option value="">전체 하위메뉴</option>
					<c:forEach items="${boardSub }" var="model2">
						<option value="${model2 }">${model2 }</option>
					</c:forEach>
		    </select>
			</th>	
			<th class="hasinput">
				<input type="text" class="form-control search-control" data-searchindex="4" placeholder="제목 검색" />
			</th>	
			<th class="hasinput">
			...
			<th class="hasinput">
				<input type="text" class="form-control search-control" data-searchindex="7" placeholder="삭제일 검색" />
			</th>
			<th class="hasinput">
				<input type="text" class="form-control search-control" data-searchindex="8" placeholder="IP 검색" />
			</th>
		</tr>
		<%-- 화살표 정렬 --%>
		<tr>
			<th class="arrow" data-index="1">번호 </th>
			<th class="arrow" data-index="2">상위메뉴</th>
			<th class="arrow" data-index="3">메뉴</th>
			<th class="arrow" data-index="4">제목</th>
			<th class="arrow" data-index="5">작성자</th>
			<th class="arrow" data-index="6">작성일</th>
			<th class="arrow" data-index="7">삭제일</th>
			<th class="arrow" data-index="8">삭제자 IP</th>
		</tr>
	</thead>
	<tbody>
		<c:if test="${empty resultList }">
		...
		<tr>
			<td>${model.NUM}</td>
			<td>${model.MN_MAIN_NAME}</td>
			<td>${model.MN_NAME}</td>
			<td>
				<a href="#" onclick="pf_DetailMove('${model.BN_KEYNO}');"><c:out value="${model.BN_TITLE}"/></a>
			</td>
