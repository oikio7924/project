...
<body>
	...
	<table>
		<thead>
			<tr></tr>
			<tr class="background">
				<th colspan="${colspan}" class="title">도메인 리스트</th>
			</tr>
			<tr></tr>
			<tr>
				<th colspan="${colspan}" class="Subdate" style="text-align: left;">
					검색결과 : ${fn:length(resultList)}건</th>
	...
	<table id="dt_basic">
		<thead>
			<tr class="background2">
				...
				<th class="thsize">그룹</th>
				<th class="thsize">도메인명</th>
				<th class="thsize">언어</th>
				<th class="thsize">메뉴 깊이제한</th>
				<th class="thsize">URL</th>
				<th class="thsize">사이트 경로</th>
				<th class="thsize">정렬순서</th>
		...
		<tbody>
			<c:forEach items="${resultList }" var="model" varStatus="status">
				<tr>
					...
					<td class="tdName">${model.HOMEPAGE_REP }</td>
					<td class="tdName">${model.HM_GROUP_NAME }</td>
					<td class="tdName">${model.MN_NAME }</td>
					<td class="tdName">${model.HM_LANG }</td>
					<td class="tdName">${model.HM_MENU_DEPTH }</td>
					<td class="tdName">${model.HM_TILES }</td>
					<td class="tdName">${model.HM_SITE_PATH }</td>
					<td class="tdName">${model.MN_ORDER }</td>
					<td class="tdName">${model.HM_USE_YN }</td>
				</tr>
			</c:forEach>
		</tbody>
	</table>