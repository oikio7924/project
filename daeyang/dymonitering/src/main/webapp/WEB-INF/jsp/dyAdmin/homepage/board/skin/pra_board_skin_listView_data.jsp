...
<table id="dt_basic" class="pagingTable table table-striped table-bordered table-hover" width="100%">
	<thead>
		...
		<tr>
			...
			<th class="arrow" data-index="2">유형</th>
	...
	<tbody>
		...
		<tr>
			...
			<td><c:out value="${model.BST_TYPE_NAME} "/></a></td>
			<td><a href="javascript:pf_boardTemplateUpdate('${model.BST_KEYNO}')"><c:out value="${model.BST_TITLE} "/></a></td>
			<td><c:out value="${model.BST_UI_NAME} "/></td>
			<td><c:out value="${model.BST_REGDT }"/></td>
			<td>
				<div class="btn btn-warning btn-xs" onclick="pf_PublishFile('${model.BST_KEYNO}')">
					<i class="fa fa-file"></i> 배포하기
				</div>
			</td>
			<td><button type="button" onclick="pf_fileNumberCopy('${model.BST_KEYNO}')"><c:out value="${model.BST_KEYNO }"/></button></td>
			<td>
				<div class="btn btn btn-default btn-xs" onclick="pf_Copy('${model.BST_KEYNO}')">
