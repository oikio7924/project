...
<table id="dt_basic" class="pagingTable table table-striped table-bordered table-hover" width="100%">
	...
	<tbody>
		...
		<tr>
			...
			<td><a href="javascript:pf_categoryUpdate('${model.TCGM_KEYNO}')"><c:out value="${model.TCGM_TITLE} "/> </a></td>
			<td><c:out value="${model.TCGM_UI_NAME} "/></td>
			<td><c:out value="${model.TCGM_REGDT } "/></td>
			<td>
				<a class="btn btn-warning btn-xs" href="#" onclick="pf_CreateFile('${model.TCGM_KEYNO}')">
					<i class="fa fa-file"></i> 배포하기
				</a>
			</td>
			<td>
				<a class="btn btn btn-default btn-xs" href="#" onclick="pf_Copy('${KEY}');">
