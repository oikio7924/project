...
String fileName = request.getParameter("MN_NAME") +  " 삭제 게시물 내역.xls";
...
<table id="dt_basic">
	...
	<tbody>
		...
		<tr>
			...
			<td class="tdName">${model.MN_MAIN_NAME}</td>
			<td class="tdName">${model.MN_NAME}</td>
			<td class="tdName">${model.BN_TITLE }</td>
			<td class="tdName"><c:out value="${model.BN_UI_NAME} "/></td>
			<td class="tdName">${model.BN_REGDT }</td>
			<td class="tdName">${model.BN_DELDT }</td>
			<td class="tdName">${model.BN_DELETE_IP }</td>
		</tr>
		</c:forEach>
	</tbody>
</table>
