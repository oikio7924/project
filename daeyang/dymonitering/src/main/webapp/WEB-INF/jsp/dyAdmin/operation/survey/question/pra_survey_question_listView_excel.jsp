...
<table>
	<thead>
		<tr></tr>
		<tr class="background">
			<th colspan="${colspan}" class="title">문항 리스트</th>
		</tr>
		<tr></tr>
		<tr>
...
<table id="dt_basic">
	<thead>
		<tr class="background2">
			...
			<th class="thsize">타입</th>
			<th class="thsize">질문</th>
			<th class="thsize">카테고리 텍스트</th>
			<th class="thsize">문항 설명</th>
			<th class="thsize">기타의견 유무</th>
		</tr>
	</thead>
	<tbody>
		<c:forEach items="${SQ }" var="model" varStatus="status">
		<tr>
			<td class="tdName">${model.COUNT }</td>
			<c:if test="${model.SQ_ST_TYPE eq 'T'}" >
				<td class="tdName">주관식(텍스트박스)</td>
			</c:if>
			<c:if test="${model.SQ_ST_TYPE eq 'R'}" >
				<td class="tdName">객관식(라디오박스)</td>
			</c:if>			
			<c:if test="${model.SQ_ST_TYPE eq 'C'}" >
				<td class="tdName">객관식(체크박스)</td>
			</c:if>			
			<td class="tdName">${model.SQ_QUESTION }</td>
			<td class="tdName">${model.SQ_KG_TEXT }</td>
			<td class="tdName">${model.SQ_COMMENT }</td>
			<td class="tdName">${model.SQ_ORDER_TYPE }</td>
		</tr>
		</c:forEach>
	</tbody>
</table>
