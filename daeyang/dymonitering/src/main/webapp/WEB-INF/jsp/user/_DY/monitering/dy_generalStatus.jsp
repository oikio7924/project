...
<%--                         </c:forEach> --%>
                ...
                <p class="power_addr"><c:choose><c:when test="${fn:contains(ob.DPP_LOCATION,'//')}">${fn:substringAfter(ob.DPP_LOCATION,'//')}</c:when><c:otherwise>${ob.DPP_LOCATION}</c:otherwise></c:choose> </p>
                ...
                <ul class="power_info">
                    ...
                    <li>
                        ...
                        <p class="rb w100">
                            <span class="gb_txt" id="Conndate">${empty ob.DDM_DATE? '데이터 없음': ob.DDM_DATE }</span>
                        </p>
                    </li>
                </ul>
        ...
        <section class="base_pop_wrapper">
            <div class="pop_base_calculation">
                ...
                <div class="form_box">
                    ...
                    <dl>
                        <dt>가중치</dt>
                        <dd id="plus"><fmt:formatNumber value="${(ob.DPP_INVER_COUNT + 0.2)/ob.DPP_INVER_COUNT}" pattern="0.00"/></dd>
                    </dl>
                </div>
...
<%--                             	<c:forEach begin="0" end="${ob.DPP_INVER_COUNT-1 }" var="cnt"> --%>
                            		...
                            		<tr>
        	                            ...
        	                            <td><fmt:formatNumber value="${not empty model.Active_Power?model.Active_Power:0}" pattern="0.00"/></td>
	...
	$.ajax({
        ...
        success: function(result) {
        	...
        	if(other_count != "") {
        		...
        		//개별용량 값이 없거나 숫자가 아니면 총용량/인버터수 기본값을 그대로 사용
        		var individualVolum = parseFloat(volum_list[numbering-1]);
        		if(!isNaN(individualVolum) && individualVolum > 0){
        			ALL_VOLUM = individualVolum;
        		}
        	}
        	...
        	if(result.invertData == null){
            	...
            	$("#Active").text(parseFloat(result.invertData.Active_Power).toFixed(2)+"kW")
...
function plusValue(value){
	var a = "<fmt:formatNumber value="${(ob.DPP_INVER_COUNT + 0.2)/ob.DPP_INVER_COUNT}" pattern="0.00"/>"
	if(value == 'b'){
		$("#plus").text("1.5")
	}else{
		$("#plus").text(a)
		       ...
		       series: [
		           {
		               ...
		               detail: {
		                   ...
		                   fontSize: 0,
		                   fontWeight: 'bolder',
		                   formatter: function(value){ return value.toFixed(2) + ' kW'; },
		                   color: 'auto',
		                   show : 'false'
		               },
		               data: [
