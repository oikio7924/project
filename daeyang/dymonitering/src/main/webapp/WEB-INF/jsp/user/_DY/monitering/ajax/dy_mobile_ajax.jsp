                          ...
                          <tbody>
                              <c:choose>
                              	<c:when test="${DaliyType eq '1' }">
                                   ...
                                   <td class="txtR" style="width: 50%;"><b><fmt:formatNumber value="${empty mindata.Active_Power ? 0 : mindata.Active_Power}" pattern="0.00"/></b>kW</td>
                               </tr>	
                              	</c:when>
                              	<c:otherwise>
          							<tr>
                      ...
                      <table class="tbl_normal auto">
                          <tbody>
                              <c:choose>
                          		...
                          		<tr>
                                   <th class="txtL" style="width: 50%;">발전 출력</th>
                                   <td class="txtR" style="width: 50%;"><b><fmt:formatNumber value="${empty maxdata.Active_Power? 0: maxdata.Active_Power}" pattern="0.00"/></b>kW</td>
                               </tr>	
                              	</c:when>
                              	<c:otherwise>
          							<tr>
...
<a id="more" href="javascript:;" style="padding: 0px 0px 0px 10px;font-size: 15px;margin-top: 7px;float: left;" onclick="moreTable('${ob.DPP_KEYNO}');">상세보기</a>
            ...
            <c:forEach items="${result}" var="result">
             ...
             <c:choose>
             	<c:when test="${DaliyType eq '1' }">
                  ...
                  <td><fmt:formatNumber value="${result.Active_Power}" pattern="0.00"/></td>
             	</c:when>
...
<!-- 상세보기 팝업 wrapper (moreTable()이 여기에 dy_statstics_ajax2.jsp 내용을 로드) -->
<section class="base_pop_wrapper"></section>

<script>
// 상세보기 - 인버터/MPPT 원본 데이터 표 (탭 구성) 팝업
function moreTable(keyno){
	$.ajax({
...
// 발전 출력 그래프 - 화면 너비/줌 비율 변경 시 그래프도 같이 리사이즈
window.addEventListener('resize', function(){
	var dom = document.getElementById('line_graph');
	var chart = dom && echarts.getInstanceByDom(dom);
