	...
	function checkDragAndDropAPI(){
		try{
			...
			if(oNavigator.mobile || oNavigator.msafari ){
				bSupportDragAndDropAPI = false;
			}else if( !oNavigator.ie ){
    ...
    function callAjaxForHTML5 (tempFile, sUploadURL){
		...
		oAjax.header("menuName",encodeURIComponent(opener.parent.menuName));
	 ...
	 */
 	function setPhotoToEditor(oFileInfo){
 		if (!!opener && !!opener.nhn && !!opener.nhn.husky && !!opener.nhn.husky.PopUpManager) {
			...
			var tempImgs = "";
			for(var i in oFileInfo){
                tempImgs += '<img src="' + oFileInfo[i].sFileURL + '" alt="' + oFileInfo[i].sFileName + '" style="max-width:95%;"/>';
			}
			opener.nhn.husky.PopUpManager.setCallback(window, 'PASTE_HTML', [tempImgs]);
		}
	}
