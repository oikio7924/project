$(() => {
  $("#sidebar-menu, #transitionBg").on("click", () => {
    tailwindTransition("#transition-sidebar")
    tailwindTransition("#transitionBg")
  })
  
  var month = document.getElementsByClassName("month_btn");
  var real = document.getElementsByClassName("relative");
  $(".month_btn").on("click", (e) => {    
		for(var i = 0; i<month.length; i++){		
	        $(`label[for=month_`+i+`]`).addClass("text-black bg-badge-green")
	        $(`label[for=month_`+i+`]`).removeClass("text-white bg-active-green")
		}
  ...
  $(".relative a").on("click", (e) => {    
		...
		for(var i = 0; i<real.length; i++){		
	        $(".relative a").addClass("text-black bg-button-gray")	
	        $(".relative a").removeClass("text-white bg-active-blue")
		}
		$(e.target).addClass("text-white bg-active-blue")
      	$(e.target).removeClass("text-black bg-button-gray")
  })

	$("#excelFile").on("change",function(){
	  var fileName = $("#excelFile").val();
	  var idx = fileName.lastIndexOf("\\");
	  var result  = fileName.substring(idx+1)
	  $(".upload-name").val(result);
	});
	
	
})
function tailwindTransition(selector){
  const entering = {
    transition: "transition ease-in-out duration-150",
    hiddenFrom: 'hidden',
    hiddenTo: '',
    from: "opacity-0 translate-y-1 -z-10 hidden",
    to: "opacity-100 translate-y-0 z-50"
  }
  const leaving =  {
    transition: "transition ease-in-out duration-150",
    hiddenFrom: '',
    hiddenTo: 'hidden',
    from: "opacity-100 translate-y-0 z-50",
    to: "opacity-0 translate-y-1 -z-10 hidden",
  }

  const transitionSidebar = $(selector)
  const displayStyle = transitionSidebar.hasClass(leaving.from) ? "none" : "block"
  transitionSidebar.attr("style", `display: ${displayStyle};`)
  let direction = transitionSidebar.hasClass(leaving.from) ? leaving : entering
  
  transitionSidebar.addClass(direction.transition).removeClass(direction.from).addClass(direction.to)
  setTimeout(_ => transitionSidebar.removeClass(direction.transition), 250)
}
