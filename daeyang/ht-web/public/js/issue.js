...
let issueKindFilterState = "";
let issueTransmitFilterState = "";
...
function issueKindFilterValue() {
  return issueKindFilterState;
}

function issueTransmitFilterValue() {
  return issueTransmitFilterState;
}
...
function recipientTransmitKey(rec, page) {
  ...
  const status = page.transmitStatus;
  if (status === "transmitted_practice") return "transmitted";
  if (status === "issued") return "issued";
  if (status === "transmit_failed") return "failed";
  if (status === "issue_failed") return "issue_failed";
  if (status === "not_sent" && page.recordId != null) return "pending";
...
function issueSearchMatches(rec, page, raw) {
  ...
  if (col === "amount") {
    const amountText = raw.replace(/,/g, "").replace(/원$/, "");
    if (!/^\d+$/.test(amountText)) return false;
    return calcRecipientTotal(rec) === Number(amountText);
  }
...
function syncIssueFilterToggle(toggleId, panelId, baseLabel) {
  const toggle = document.getElementById(toggleId);
  const panel = document.getElementById(panelId);
  if (!toggle || !panel) return;
  const active = panel.querySelector(".search-col-chip.active");
  const hasFilter = !!(active && active.getAttribute("data-value"));
  toggle.textContent = hasFilter ? baseLabel + ": " + active.textContent.trim() : baseLabel;
  toggle.classList.toggle("active", hasFilter);
}

function bindIssueSubfilter(toggleId, panelId, baseLabel, onChange) {
  const toggle = document.getElementById(toggleId);
  const panel = document.getElementById(panelId);
  if (!toggle || !panel) return;
  ...
  toggle.addEventListener("click", function () {
    ...
    document.querySelectorAll(".issue-subfilter-row").forEach(function (row) { row.hidden = true; });
    document.querySelectorAll(".issue-filter-toggle").forEach(function (btn) { btn.setAttribute("aria-expanded", "false"); });
    panel.hidden = !nextOpen;
    toggle.setAttribute("aria-expanded", nextOpen ? "true" : "false");
  });

  panel.addEventListener("click", function (e) {
    const chip = e.target.closest(".search-col-chip[data-value]");
    if (!chip) return;
    panel.querySelectorAll(".search-col-chip").forEach(function (btn) { btn.classList.remove("active"); });
    chip.classList.add("active");
    onChange(String(chip.getAttribute("data-value") || ""));
    syncIssueFilterToggle(toggleId, panelId, baseLabel);
    panel.hidden = true;
...
function bindIssueFilterRow(panelId, onChange) {
  const panel = document.getElementById(panelId);
  if (!panel) return;
  panel.addEventListener("click", function (e) {
    const chip = e.target.closest(".search-col-chip[data-value]");
...
function transmitStatusDisplay(rec, page) {
  ...
  if (savedArr && savedArr.length > 0) {
    ...
    if (allFailed) {
      const failedStatuses = savedArr.map(function (r) { return r.transmitStatus; });
      if (failedStatuses.every(function (s) { return s === "issue_failed"; })) return "발급실패";
      if (failedStatuses.every(function (s) { return s === "transmit_failed"; })) return "전송실패";
      return "전송/발급실패";
    }
    const ss = active.map(function (r) { return r.transmitStatus; });
    if (ss.every(function (s) { return s === "issued"; })) return "발급완료";
    if (ss.every(function (s) { return s === "transmitted_practice"; })) return "전송완료";
    if (ss.every(function (s) { return s === "issued" || s === "transmitted_practice"; })) return "발급완료 포함";
...
function calcRecipientTotal(rec) {
  const totals = recipientTotals(rec);
  return totals.supply + totals.tax;
}
...
function updateIssueTotals(visibleTotal) {
  ...
  if (selEl) {
    ...
    selectedRecipientIds.forEach(function (id) {
      ...
      if (rec) selTotal += calcRecipientTotal(rec);
    });
...
function renderSessionTable() {
  ...
  for (let i = 0; i < allRecipients.length; i++) {
    ...
    const totalAmount = calcRecipientTotal(rec);
...
function renderSingleInvoice() {
  ...
  if (!invoicePages.length) {
    ...
    el.innerHTML = '<div class="invoice-empty-hint"><p>선택된 항목이 없습니다.</p><p>왼쪽 목록에서 항목을 선택하세요.</p></div>';
    syncTransmitButtonState();
    return;
  }
...
(async function () {
  ...
  bindIssueFilterRow("issue-kind-subfilters", function (value) {
    issueKindFilterState = value;
  });
  bindIssueFilterRow("issue-transmit-subfilters", function (value) {
    issueTransmitFilterState = value;
  });
  const searchFilterToggle = document.getElementById("issue-search-filter-toggle");
  const searchFilterPanel = document.getElementById("issue-search-filter-panel");
  if (searchFilterToggle && searchFilterPanel) {
    searchFilterToggle.addEventListener("click", function () {
      const willOpen = searchFilterPanel.classList.contains("is-collapsed");
      searchFilterPanel.classList.toggle("is-collapsed");
      searchFilterToggle.setAttribute("aria-expanded", willOpen ? "true" : "false");
      searchFilterToggle.textContent = willOpen ? "검색필터 ▲" : "검색필터 ▼";
    });
  }
  if (ymInputEl) {
