...
let matrixMonthFilter = "";
let matrixStatusFilter = "";
...
function statusMatchesFilter(ts, filter) {
  if (filter === "failed") return ts === "transmit_failed" || ts === "issue_failed";
  if (filter === "none") return !ts || ts === "none";
  return ts === filter;
}
...
function matchesAllFilters(row) {
  ...
  if (f.name) {
    if (col === "bizno") {
      ...
      const mrs = Array.isArray(row.monthRecords) ? row.monthRecords : [];
      const mr = mrs[currentMonth];
      const thisMonthTotal = mr ? (Number(mr.totalSupply) || 0) + (Number(mr.totalTax) || 0) : 0;
      const qDigits = f.name.replace(/[^0-9]/g, "");
      if (!qDigits) return true;
      if (String(thisMonthTotal) !== qDigits && formatMoney(thisMonthTotal).replace(/,/g, "") !== qDigits) return false;
    } else {
  ...
  if (matrixMonthFilter !== "" || matrixStatusFilter !== "") {
    const months = Array.isArray(row.months) ? row.months : [];
    const mIdx = matrixMonthFilter !== "" ? Number(matrixMonthFilter) : -1;
    if (mIdx >= 0 && matrixStatusFilter !== "") {
      if (!statusMatchesFilter(months[mIdx] || "none", matrixStatusFilter)) return false;
    } else if (mIdx >= 0) {
      const ts = months[mIdx] || "none";
      if (!ts || ts === "none") return false;
    } else {
      const hasMatch = months.some(function (ts) {
        return statusMatchesFilter(ts || "none", matrixStatusFilter);
      });
      if (!hasMatch) return false;
    }
  }
...
function renderMatrixRows() {
  ...
  const sumTotal = filtered.reduce(function (s, r) {
    const mrs = Array.isArray(r.monthRecords) ? r.monthRecords : [];
    let latestIssued = null;
    for (let m = 11; m >= 0; m--) {
      if (mrs[m] && (mrs[m].transmitStatus === "issued" || mrs[m].transmitStatus === "transmitted_practice")) { latestIssued = mrs[m]; break; }
    }
    const amount = latestIssued
      ? (Number(latestIssued.totalSupply) || 0) + (Number(latestIssued.totalTax) || 0)
      : (Number(r.total) || 0);
    return s + amount;
  }, 0);
  ...
  for (let i = 0; i < filtered.length; i++) {
    ...
    const monthRecordsForDisplay = Array.isArray(r.monthRecords) ? r.monthRecords : [];
    let latestIssuedRecord = null;
    for (let m = 11; m >= 0; m--) {
      const mr = monthRecordsForDisplay[m];
      if (mr && (mr.transmitStatus === "issued" || mr.transmitStatus === "transmitted_practice")) { latestIssuedRecord = mr; break; }
    }
    const tdTotal = document.createElement("td");
    tdTotal.textContent = latestIssuedRecord
      ? formatMoney((Number(latestIssuedRecord.totalSupply) || 0) + (Number(latestIssuedRecord.totalTax) || 0)) + "원"
      : formatMoney(r.total) + "원";
    const tdSupply = document.createElement("td");
    tdSupply.textContent = latestIssuedRecord
      ? formatMoney(Number(latestIssuedRecord.totalSupply) || 0) + "원"
      : formatMoney(r.supply) + "원";
  ...
  if (countEl) {
    countEl.textContent =
      String(filtered.length) + (isPlantView ? "개소" : "건");
  }
}
...
(async function () {
  ...
  const monthFilterEl = document.getElementById("history-month-filter");
  if (monthFilterEl) {
    monthFilterEl.addEventListener("change", function () {
      matrixMonthFilter = this.value;
      renderMatrixRows();
    });
  }

  const statusChipsEl = document.querySelector(".history-status-chips");
  if (statusChipsEl) {
    statusChipsEl.addEventListener("click", function (e) {
      const chip = e.target.closest(".history-status-chip");
      if (!chip) return;
      const status = chip.getAttribute("data-status");
      if (matrixStatusFilter === status) {
        matrixStatusFilter = "";
        statusChipsEl.querySelectorAll(".history-status-chip").forEach(function (b) {
          b.classList.remove("active");
        });
      } else {
        matrixStatusFilter = status;
        statusChipsEl.querySelectorAll(".history-status-chip").forEach(function (b) {
