...
function renderDashboard(data) {
  ...
  if (strip && data.periods) {
    ...
    strip.innerHTML =
      ...
      "건</span></div>" +
      '<div class="dash-summary-row" style="' + cr + '" data-summary-cat="issued"><span class="dash-summary-name">발급완료</span><span class="dash-summary-value">' +
      (Number(tm.issued) || 0) +
      "건</span></div>" +
      ...
      "건</span></div>" +
      '<div class="dash-summary-row" style="' + cr + '" data-summary-cat="issue-failed"><span class="dash-summary-name">발급실패</span><span class="dash-summary-value">' +
      (Number(tm.issueFailed) || 0) +
      "건</span></div>" +
...
function filterSummaryRecords(category) {
  return dashThisMonthRecords.filter(function (r) {
    if (category === "issued") return r.transmitStatus === "issued";
    if (category === "transmitted") return r.transmitStatus === "transmitted_practice";
    if (category === "pending") return r.transmitStatus === "not_sent";
    if (category === "failed") return r.transmitStatus === "transmit_failed";
    if (category === "issue-failed") return r.transmitStatus === "issue_failed";
    return true;
  });
}
...
function renderSummaryTab(category) {
  ...
  thead.innerHTML = "<tr><th>발전소명</th><th>사업주</th>" + (isAll ? "<th>전송상태</th>" : "") + "</tr>";

  const list = filterSummaryRecords(category);
  tbody.innerHTML = "";
  if (!list.length) {
    tbody.innerHTML = "<tr><td colspan='" + (isAll ? 3 : 2) + "' style='color:var(--text-muted)'>해당 항목이 없습니다.</td></tr>";
    return;
  }
  list.forEach(function (r) {
    var tr = document.createElement("tr");
    var cols = [(r.plantNames || []).join(", ") || "-", r.ceoName || "-"];
