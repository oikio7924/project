...
function sumIssueRecords(list) {
  ...
  for (let i = 0; i < list.length; i++) {
    ...
    if (r.transmitStatus === "issued") issued += 1;
    else if (r.transmitStatus === "transmitted_practice") transmitted += 1;
    else if (r.transmitStatus === "transmit_failed") transmitFailed += 1;
    else if (r.transmitStatus === "issue_failed") issueFailed += 1;
  ...
  return {
    ...
    transmitFailed,
    issueFailed,
    notTransmitted: pendingTransmit + transmitFailed + issueFailed,
  };
}
...
// 서버 재시작 시 transmitted_practice 상태 레코드의 폴링을 재개
async function resumePendingPolls() {
  const provider = String(process.env.HOMETAX_INVOICE_PROVIDER || "").trim().toLowerCase();
  if (provider !== "hometaxbill") return;
  try {
    const rows = await q(
      "SELECT id, user_id, issue_date, recipient_id FROM issue_records WHERE transmit_status = 'transmitted_practice'"
    );
    if (!rows.length) return;
    console.log("[resumePendingPolls] transmitted_practice 레코드 " + rows.length + "건 폴링 재개");
    for (let i = 0; i < rows.length; i++) {
      const r = rows[i];
      const homemunseoId = buildHomemunseoId(r.issue_date, r.recipient_id, r.id);
      pollHometaxStatus(Number(r.id), Number(r.user_id), homemunseoId, 1);
    }
  } catch (e) {
    console.error("[resumePendingPolls] 오류:", e);
  }
}
